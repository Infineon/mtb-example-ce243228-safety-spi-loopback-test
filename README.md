# Safety Test Library (STL) : SPI

This code example demonstrates the usage of ModusToolbox&trade; Safety Test Library (MTB STL) to test the SPI peripheral of the MCU using a hardware internal loopback.
...
[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce243228-safety-spi-loopback-test)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The SPI self-test uses the SCB1 peripheral with the hardware internal loopback feature enabled. No external wiring between the MOSI and MISO pins is required.
...
The following example clones the "[SPI loopback test](https://github.com/Infineon/mtb-example-ce243228-safety-spi-loopback-test)" application with the desired name "SPI_Loopback_Test" configured for the *KIT_XMC52_EVK* BSP into the specified working directory *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC52_EVK --app-id mtb-example-ce243228-safety-spi-loopback-test --user-app-name SPI_Loopback_Test --target-dir "C:/mtb_projects"
   ```
...
4. After programming, the application starts automatically. Confirm that **MTB STL: SCB SPI Safety Test** is displayed on the UART terminal.

5. The serial terminal should display the result of the test.

   **Figure 1. Terminal output on program startup**

   ![](images/terminal-spi-loopback-test.png)
...
This example uses the SCB SPI peripheral on the MCU to perform self-test of the SCB peripheral. The design.modus file configures the DUT SPI on SCB1.

The example begins by initializing the BSP configuration and setting up retarget-io for debug prints over the dedicated debug UART. The DUT SPI (SCB1) is then initialized and the hardware internal loopback mode is enabled once at init by setting the `SCB_SPI_CTRL.LOOPBACK` bit. This routes the MOSI output directly back to the MISO input inside the SCB hardware block, so no external wiring between the MOSI and MISO pins is required.

A self-test is then run using the `SelfTest_SPI_SCB()` API from the mtb-stl (https://github.com/Infineon/mtb-stl) safety library. The library transmits a byte pattern and verifies the received data against the expected values. Because the function advances an internal byte counter on each call and returns `PASS_STILL_TESTING_STATUS` until all bytes have been verified, the application loops calling `spi_self_test()` until a final pass or fail status is returned. The FIFOs are flushed before and after each iteration. The PASS or FAIL result is printed to the debug UART console, and the device halts after a single complete run.
...
**Table 1. Application resources**
 ...
 SCB (PDL) | CYBSP_DUT_SPI | DUT SCB1 SPI peripheral under test (hardware internal loopback)
...
Document title: *CE243228* – *Safety Test Library (STL) : SPI Loopback Test*
